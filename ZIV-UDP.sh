...
case $selected_option in
    1)
        ...
        if [ -n "$input_config" ]; then
            ...
            echo -e "${YELLOW}Enter auth separated by commas${NC}"
            exit 1
        fi
        ...
        while true; do
            ...
            if is_number "$remote_udp_port" && [ "$remote_udp_port" -ge 1 ] && [ "$remote_udp_port" -le 65534 ]; then
                if netstat -tulnp | grep -q "::$remote_udp_port"; then
                    echo -e "${YELLOW}Error: The selected port has already been used${NC}"
                else
                    break
                fi
            else
        ...
        if [ ! -e "$file_path" ]; then
            echo -e "${YELLOW}Error: Unable to save the config.json file${NC}"
            exit 1
        fi
